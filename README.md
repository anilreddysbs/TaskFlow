...
This project is configured for a multi-service deploy to **Render** using a Render Blueprint ([render.yaml](file:///c:/Users/USER/Desktop/Django/djnago_youtube/taskflow/render.yaml)).

### Services Deployed:
1. **Web Service**: Gunicorn serving the Django API (with WhiteNoise for static files).
2. **PostgreSQL**: Production-grade relational database.
3. **Redis**: Cache storage and Celery message broker.
4. **Celery Worker**: Background task executor.
...
5. Provide a group name for the Blueprint.
6. Render will automatically detect the `render.yaml` file, provision the PostgreSQL and Redis instances, build the Docker images, and set up all environment variables.
7. Click **Apply** to deploy the services.

### Environment Configuration:
Render handles configuration automatically using environment variables specified in the blueprint:
- `DJANGO_SECRET_KEY`: Automatically generated by Render.
- `DATABASE_URL`: Automatically linked to the provisioned PostgreSQL instance.
- `REDIS_URL` & `CELERY_BROKER_URL`: Automatically linked to the provisioned Redis instance.
- `ALLOWED_HOSTS`: Set to `*` by default and dynamically includes the Render Web Service hostname at runtime.
- `DEBUG`: Defaulted to `False` for production safety.

For other cloud deployment targets (like AWS), refer to [PRODUCTION_GUIDE.md](docs/PRODUCTION_GUIDE.md) for custom VPC, RDS, and ECS container setups.
